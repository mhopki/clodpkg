...
class POCont:
    ...
    def odom_callback(self, data):
        ...
        if self.r_theta == -100.0:
            self.r_theta = np.arcsin(data.pose.pose.orientation.z) * 2.0
    ...
    def set_rtheta(self):
        if abs(self.r_theta) < 99:  # Check if orientation is valid
            des_t = abs(np.arcsin(self.r_odom.pose.pose.orientation.z) * 2.0)  # Desired orientation
    ...
    def calculate_desired_cmd(self, current_pose, waypoint_pose, kp_linear, kp_angular, targ_vel):
        ...
        #print("DES[", self.robot_number, "]: ", desired_heading)
        
        # Calculate the difference between current and desired heading
        heading_error = desired_heading - (np.arcsin(current_pose.pose.pose.orientation.z) * 2.0)#(current_pose.pose.pose.orientation.z * math.pi)
        #print("HERR[", self.robot_number, "]: ", heading_error)
    ...
    def spin(self):
        ...
        while not rospy.is_shutdown():
            ...
            fixed_odom.pose.pose.orientation.z = np.sin(0.5 * self.r_theta)  # Update the robot's orientation angle
            ...
            anggain = 1.0 * (1.0 - (np.clip((speed - 0.45),0,3.0) * 0.5)) #1.0 * (1.0 - (np.clip((speed - 0.6),0,2.0) * 0.68)) 0.68 perfect video #0.65 #0.68 #<- new pefrect 1.5 #0.65#0.6 <- 1.0#1.5 perfect:1.0 * (1.0 - (np.clip((speed - 0.6),0,2.0) * 0.72))#02: 1.0
            #print("SPEED: ", speed)
            #print("ANG: ", anggain)
            ...
            # If the robot is traveling towards the goal but has not reached it
                ...
                if lin < 0:
                    lin *= 2.0 #2.2 perf video #1 perf:2.5 #0.5: 1.5
                    if lin < -0.01:
                        lin = -0.01
                if lin > 0:
                    #lin / = 2
                    lin *= 2.0 #2.2 perf video#1 perf:2.5 #0.5: 4.5 #1.5
                    if lin > 0.01:
                        lin = 0.01
                ...
                ang_out_f = -((ang / 2) - 0.023) *4 #0.028  # + 0.025) * 4 
                if ang_out_f > 1.0:
                    ang_out_f = 1.0
                if ang_out_f < -1.0:
                    ang_out_f = -1.0
