...
class POCont:
	def __init__(self):
		...
		self.g_loc = [0.2, 5]
		self.g_odom.pose.pose.position.x = 0.2 #self.g_loc[0]
		self.g_odom.pose.pose.position.y = 5 #self.g_loc[1]
	...
	def spin(self):
		...
		while not rospy.is_shutdown():
			...
			if (self.r_pos[0] < -1.5 or self.r_pos[0] > 1.5 or self.r_pos[1] > 6 or self.r_pos[1] < 1):
				...
				self.joy_msg.buttons = [0] * 12
				print("OB")
			elif (abs(self.r_pos[0] - self.g_loc[0]) > 0.1 or abs(self.r_pos[1] - self.g_loc[1]) > 0.3):
				#WE ON GO
				lingain = 1.0
				anggain = 1.2
			...
			elif (abs(self.r_pos[0] - self.g_loc[0]) < 0.1 and abs(self.r_pos[1] - self.g_loc[1]) <= 0.3):
				#WE MADE IT NIGGA!!!
				self.joy_msg = Joy()
				self.joy_msg.axes = [0.0] * 8
				self.joy_msg.buttons = [0] * 12
